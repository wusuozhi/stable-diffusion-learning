...
def hf_dataset(
    ...
    """
    ds = load_dataset(name, split=split,cache_dir='hg_data/')
...
def pokemon_local_ds(
    ...
    img_paths = [ os.path.abspath(a) for a in df['img_path'].tolist()]
    df_dict = {'image':img_paths,'txt':df['text'].tolist()}
    ds = hg_Dataset.from_dict(df_dict)
